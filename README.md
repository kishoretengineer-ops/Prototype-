...
Verity Voice is a polished AI voice screening interface backed by **FastAPI**,
PyAV, and a lazily loaded Hugging Face audio-classification model. It accepts
WAV, MP3, M4A, and browser-recorded WebM audio, normalizes it to 16 kHz mono,
and analyzes at most the first 60 seconds. Results are probabilistic screening
...
The API is available at `http://127.0.0.1:8000`. The first analysis downloads
the public `MelodyMachine/Deepfake-audio-detection-V2` model and may take a
little longer. If the model cannot load, Verity Voice fails closed and asks you
to retry; it never substitutes a mocked classification.

## Template development

The hosted split app runs with the template's supervisor programs: FastAPI on
port 8001 and the Vite frontend on port 3000. The frontend uses the relative
`/api` proxy and the same route is also exposed through `app.main:app` for the
local API command above.
